    ...
    <v-app>
        ...
        <v-main :class="{ 'main-collapsed': isSidebarCollapsed }">
            <v-container fluid class="pa-5 bg-white">
                <v-row no-gutters>
                    <v-col class="d-flex align-center pa-4">
                        ...
                        <span class="font-weight-bold text-h6 ml-5">List Aplikasi </span>
                    </v-col>
                </v-row>
            </v-container>
            <v-divider></v-divider>
            <v-container fluid class="my-3">
                        ...
                        <v-card>
                            <v-card-title class="d-flex justify-space-between my-5">
                                <div class="d-flex align-center">
                                    <Icon icon="icon-park-outline:cube" style="color: black" width="24" />
                                    <div class="font-weight-bold ml-2">Data Aplikasi Client</div>
                                </div>
                                <v-btn color="teal" dark @click="showAddAppDialog">
                ...
                <AddAplikasi :isVisible="isAddAplikasiVisible" @update:isVisible="isAddAplikasiVisible = $event" />
            </v-container>
        </v-main>
...
import AddAplikasi from '@/components/Dialog/AddAplikasi.vue';
...
export default {
    ...
    setup() {
        const isAddAplikasiVisible = ref(false);

        const showAddAppDialog = () => {
            isAddAplikasiVisible.value = true;
        };

        return {
            isAddAplikasiVisible,
            showAddAppDialog,
        };
    },
    methods: {
